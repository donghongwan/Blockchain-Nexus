...
class BankOfAmericaAPI:
    def __init__(self):
        self.headers = {
            "Authorization": f"Bearer {Config.BANK_OF_AMERICA_API_KEY}",
            "Content-Type": "application/json"
        }

    def get_account_balance(self, account_id):
        url = f"{Config.BANK_OF_AMERICA_BASE_URL}/accounts/{account_id}/balance"
    ...
    def get_transaction_history(self, account_id):
        url = f"{Config.BANK_OF_AMERICA_BASE_URL}/accounts/{account_id}/transactions"
        response = requests.get(url, headers=self.headers)
