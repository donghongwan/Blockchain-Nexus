...
class NABAPI:
    def __init__(self):
        self.headers = {
            "Authorization": f"Bearer {Config.NAB_API_KEY}",
            "Content-Type": "application/json"
        }

    def get_account_balance(self, account_id):
        url = f"{Config.NAB_BASE_URL}/accounts/{account_id}/balance"
    ...
    def get_transaction_history(self, account_id):
        url = f"{Config.NAB_BASE_URL}/accounts/{account_id}/transactions"
        response = requests.get(url, headers=self.headers)
